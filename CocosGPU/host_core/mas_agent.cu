...
MasAgent::search_alloc ( int max_beam_size ) {
  ...
  /// Dom_size_1, <phi, psi>, ..., <phi, psi>, Dom_size_2, ...
  HANDLE_ERROR( cudaMalloc( ( void** )&gd_params.all_domains, (_n_res + (2 * _sum_dom_size)) * sizeof( real ) ) );
  HANDLE_ERROR( cudaMalloc( ( void** )&gd_params.all_domains_idx, _n_res * sizeof( int ) ) );
  HANDLE_ERROR( cudaMalloc( ( void** )&gd_params.curr_str, _n_points * sizeof( real ) ) );
  HANDLE_ERROR( cudaMalloc( ( void** )&gd_params.beam_str, gh_params.set_size * _n_points * sizeof( real ) ) );
  HANDLE_ERROR( cudaMalloc( ( void** )&gd_params.beam_energies, gh_params.set_size * sizeof( real ) ) );
...
MasAgent::search_init () {
  ...
  HANDLE_ERROR( cudaMemcpy( gd_params.all_domains, all_doms,
                           (_n_res + (2 * _sum_dom_size)) * sizeof(real), cudaMemcpyHostToDevice ) );
  HANDLE_ERROR( cudaMemcpy( gd_params.all_domains_idx, all_domains_idx,
                            _n_res * sizeof(int), cudaMemcpyHostToDevice ) );
  free ( all_doms );
...
MasAgent::search_free() {
  ...
  HANDLE_ERROR( cudaFree( gd_params.all_domains ) );
  HANDLE_ERROR( cudaFree( gd_params.all_domains_idx ) );
  HANDLE_ERROR( cudaFree( gd_params.curr_str ) );
  HANDLE_ERROR( cudaFree( gd_params.beam_str ) );
  HANDLE_ERROR( cudaFree( gd_params.beam_energies ) );
}//search_free
