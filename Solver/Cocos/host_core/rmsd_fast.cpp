...
using namespace Rmsd_fast;

/* vector functions using c arrays */

void
Rmsd_fast::normalize(double a[3])
{
  double  b;
...
double
Rmsd_fast::dot(double a[3], double b[3])
{
...
void
Rmsd_fast::cross(double a[3], double b[3], double c[3])
{
  a[0] = b[1]*c[2] - b[2]*c[1];
  a[1] = b[2]*c[0] - b[0]*c[2];
  a[2] = b[0]*c[1] - b[1]*c[0];
 ...
 */
void
Rmsd_fast::setup_rotation(double ref_xlist[][3],
                          double mov_xlist[][3],
                          int n_list,
                          double mov_com[3],
                          double mov_to_ref[3],
 ...
 */
int
Rmsd_fast::jacobi3(double a[3][3], double d[3], double v[3][3], int* n_rot)
{
  int count, k, i, j;
 ...
 */
int
Rmsd_fast::diagonalize_symmetric(double matrix[3][3],
 ...
 */
int
Rmsd_fast::calculate_rotation_matrix(double R[3][3],
                                     double U[3][3],
                                     double E0,
                                     double* residual)
{
...
void
Rmsd_fast::calculate_rotation_rmsd(double ref_xlist[][3],
                                   double mov_xlist[][3],
                                   int n_list,
                                   double mov_com[3],
                                   double mov_to_ref[3],
...
void
Rmsd_fast::get_rmsd ( real* beam_str, real* beam_energies,
...
Rmsd_fast::get_rmsd_aux ( real* my_prot,
  ...
  for ( int i = scope_first; i <= scope_second; i++ ) {
    for ( int j = 0; j < 4; j++ ) {
      ...
      mov_xlist[ t*4 + j ][2] = my_prot[ i * 15 + j*3 + 2 ];
    }//j
    t++;
  }//i
  
  fast_rmsd( ref_atoms, mov_xlist, 4*nres, &rmsd );
  return (real) rmsd;
}//get_rmsd_aux

void
Rmsd_fast::fast_rmsd(double ref_xlist[][3],
                     double mov_xlist[][3],
                     int n_list,
                     double* rmsd)
{
