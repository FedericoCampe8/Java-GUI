...
check_all_dist ( real * local_point_list, int* check_success, int n_threads, int print_failed_var ) {
  ...
  for (int thr = 0; thr < n_threads; thr++) {
    ...
    for ( int i = thr + 2; i < n_threads; i++ ) {
      ...
      if ( ( ((int)Math::eucl_dist( my_N,  &local_point_list[ i*15   ] ))*100 < (N_radii + N_radii  - epsilon) ) ||
           ( ((int)Math::eucl_dist( my_N,  &local_point_list[ i*15+3 ] ))*100 < (N_radii + Ca_radii - epsilon) ) ||
           ( ((int)Math::eucl_dist( my_N,  &local_point_list[ i*15+6 ] ))*100 < (N_radii + C_radii  - epsilon) ) ||
           ( ((int)Math::eucl_dist( my_N,  &local_point_list[ i*15+9 ] ))*100 < (N_radii + O_radii  - epsilon) ) ) {
      ...
      if ( ( ((int)Math::eucl_dist( my_Ca,  &local_point_list[ i*15   ] ))*100 < (Ca_radii + N_radii  - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_Ca,  &local_point_list[ i*15+3 ] ))*100 < (Ca_radii + Ca_radii - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_Ca,  &local_point_list[ i*15+6 ] ))*100 < (Ca_radii + C_radii  - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_Ca,  &local_point_list[ i*15+9 ] ))*100 < (Ca_radii + O_radii  - epsilon) ) ) {
        if ( print_failed_var >= 0 ) {
      ...
      if ( ( ((int)Math::eucl_dist( my_C,  &local_point_list[ i*15   ] ))*100 < (C_radii + N_radii  - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_C,  &local_point_list[ i*15+3 ] ))*100 < (C_radii + Ca_radii - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_C,  &local_point_list[ i*15+6 ] ))*100 < (C_radii + C_radii  - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_C,  &local_point_list[ i*15+9 ] ))*100 < (C_radii + O_radii  - epsilon) ) ) {
        if ( print_failed_var >= 0 ) {
      ...
      if ( ( ((int)Math::eucl_dist( my_O, &local_point_list[ i*15   ] ))*100 < (O_radii + N_radii  - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_O,  &local_point_list[ i*15+3 ] ))*100 < (O_radii + Ca_radii - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_O,  &local_point_list[ i*15+6 ] ))*100 < (O_radii + C_radii  - epsilon) ) ||
          ( ((int)Math::eucl_dist( my_O,  &local_point_list[ i*15+9 ] ))*100 < (O_radii + O_radii  - epsilon) ) ) {
        if ( print_failed_var >= 0 ) {
