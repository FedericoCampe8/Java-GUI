...
/// @note: | D_aa |       == gridDim.x
real
get_energy ( ss_type* secondary_s_info,
  ...
  uint len = g_target.get_bblen();
  
  real * plain_str = new real [ len * 3 + n_res  * 3 ];
  
  int idx = 0;
  real * h_atom = new real[ 3 ];
  for ( int i = 0; i < len; i++ ) {
    ...
    if ( idx && ((idx+1) % 5 == 0) ) {
      if ( idx == 4 ) {
        ...
        Math::calculate_h_atom ( &g_logicvars->var_point_list[ i - 6 ].lower_bound[ 0 ],
                                 &g_logicvars->var_point_list[ i - 4 ].lower_bound[ 0 ],
                                 &g_logicvars->var_point_list[ i - 3 ].lower_bound[ 0 ],
                                 h_atom );
        
        plain_str[ 3*idx + 0 ] = h_atom[ 0 ];
        plain_str[ 3*idx + 1 ] = h_atom[ 1 ];
        plain_str[ 3*idx + 2 ] = h_atom[ 2 ];
    ...
    plain_str[ 3*idx + 0 ] =
    g_logicvars->var_point_list[ i ].lower_bound[ 0 ];
    
    plain_str[ 3*idx + 1 ] =
    g_logicvars->var_point_list[ i ].lower_bound[ 1 ];
    
    plain_str[ 3*idx + 2 ] =
    g_logicvars->var_point_list[ i ].lower_bound[ 2 ];
