...
public class AssemblingOpPanel extends OpPanel{
    ...
    private void initComponents(UserFrame view){
        ...
        tool.setBorder(BorderFactory.createTitledBorder(null,
                "Tool Box", TitledBorder.DEFAULT_JUSTIFICATION,
    ...
    public void move(Boolean isUndo){
        ...
        view.upView.execute(Utilities.deleteConnectionString(
                        view.getModel().getDimensionFragmentsA()),
                        Defs.ASSEMBLING);
    ...
    private void distanceEvent(){
        ...
        */
        ...
        ArrayList<Fragment> frgsSelected = (ArrayList<Fragment>)view.getModel().getAllFragmentsA().clone();
    ...
    public void viewPdb(){
        ...
        String protein = Defs.PROTEINS_PATH + view.getModel().idProteinCode + ".in.pdb";
    ...
    public void changeViewProtein(){
        ...
        ViewOptionsPanel vop = new ViewOptionsPanel(view, Defs.ASSEMBLING);
    ...
    private void saveEvent(){
        ...
        if (cp.measures()) {
            deleteMeasures();
            ArrayList<Fragment> frgs = (ArrayList<Fragment>)fragments.clone();
            measure(frgs);
            rulerButton.setIcon(rulerOn);
            ruleOn = true;
        ...
        */
         ...
         for(int i=0;i<fragments.size();i++){
             ...
             prot = prot + "Protein ID: "+proteinId+"\n"
                     ...
                     +(Integer.parseInt(param[Defs.FRAGMENT_RESNO_START]) + 2 - firstPosition)+"\n"
                     //+(Integer.parseInt(param[Defs.FRAGMENT_END_AA]) -1 - firstPosition + 1)+"\n"
                     +(Integer.parseInt(param[Defs.FRAGMENT_RESNO_END]) - firstPosition)+"\n"
                     +"CONSTRAINTS:\n"+"NO CONSTRAINTS"+"\n\n";
         }
         try{
    ...
    public void solveEvent(){
        ...
        //Reselect fragments selected previously
        ArrayList<Fragment> frgs = view.getModel().getAllSelectedFragmentsA();
        int size = frgs.size();
        for(int i=0; i<size;i++){
            String selection = Utilities.selectFragmetString(frgs.get(i));
            ((AssemblingPanel)view.getPanel(Defs.ASSEMBLING)).executeCmd(selection);
        }
