...
public class Controller {
    ...
    private boolean loadTarget(boolean isPDB){
        ...
        if(isPDB){
            String file = "";
            HeaderPdb.info(loadPath);
            String protein = loadPath.substring(loadPath.lastIndexOf("/") + 1).split("\\.")[0].toUpperCase();
            //if(!loadPath.substring(loadPath.lastIndexOf("/") + 1).split("\\.")[1].equals("pdb"))
            //    protein += "." + loadPath.substring(loadPath.lastIndexOf("/") + 1).split("\\.")[1];
            file += ">" + protein + ":A|PDBID|CHAIN|SEQUENCE\n";
            file += HeaderPdb.getPdbTargetSequence();
            try {
                FileOutputStream fasta = new FileOutputStream(Defs.PROTEINS_PATH + protein + ".fasta");
                fasta.write(file.getBytes());
                fasta.flush();
                fasta.close();
            } catch (IOException e) {
                view.printStringLn("Error: " + e);
            }
            loadPath = Defs.PROTEINS_PATH + protein + ".fasta";
            this.isPDB = false;
            return loadTarget();
        }else{
            return loadTarget();
    ...
    public void usePickedInfo(String[] info) {
        ...
        if (currentPanel == Defs.EXTRACTION) {
            pnl = "EXTRACTION";
        } else if (currentPanel == Defs.ASSEMBLING) {
            pnl = "ASSEMBLING";
        } else {
            return;
        }
    ...
    public void saveToPdb() {
        ...
        try {
            ...
            for (int k = 0; k < fragments.size(); k++) {
                ...
                startFrg = Integer.parseInt(fragments.get(k).getParameters()[Defs.FRAGMENT_RESNO_START]);
                endFrg = Integer.parseInt(fragments.get(k).getParameters()[Defs.FRAGMENT_RESNO_END]);
