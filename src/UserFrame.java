...
public class UserFrame extends JFrame implements Observer, MouseListener, ComponentListener{
    ...
    private long w;
    private final String PROGTARGET = "  TARGET        ";
    private final String PROGEXTRACTION = "  EXTRACTION    ";
    private final String PROGASSEMBLING = "  ASSEMBLING    ";
    private final String PROGOUTPUT = "  OUTPUT    ";
    ...
    private void setupIcons(){
        ...
        /* Load and save icons */
        icons[0] = currentDir + imSuffix + "target.png";
        icons[1] = currentDir + imSuffix + "target--plus.png";
        icons[2] = currentDir + imSuffix + "disk.png";
    ...
    private void setupContent(){
        ...
        targetPanel = new TargetPanel(this, true);
   ...
   public void mouseClicked(MouseEvent e){
        ...
        if (e.getComponent().equals(assemblingPanel) ||
            ...
            bin.setSeparators(Defs.EXTRACTION, w);
        if (e.getComponent().equals(extractionPanel) ||
            e.getComponent().equals(extractionPanel.getJMolPanel()))
            bin.setSeparators(Defs.ASSEMBLING, w);
        if (e.getComponent().equals(outputPanel) ||
            e.getComponent().equals(outputPanel.getJMolPanel()))
            bin.setSeparators(Defs.OUTPUT, w);
        }
         */
    }
    @Override
   ...
   /* Print strings on the info panel */
    ...
    public void move(Boolean isUndo){
        if(isUndo){
            upView.execute("undoMove 1; ", Defs.ASSEMBLING);
        }else{
            upView.execute("redoMove 1; ", Defs.ASSEMBLING);
        }
        upView.execute("refresh; ", Defs.ASSEMBLING);
        upView.execute(Utilities.deleteConnectionString(
                        model.getDimensionFragmentsA()),
                        Defs.ASSEMBLING);
        upView.connectFragments(false);
